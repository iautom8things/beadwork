...
func TestCrossRepoFromNonBeadworkDir(t *testing.T) {
	...
	caller := &bwEnv{
		t:       t,
		dir:     nonRepo,
		cfgPath: envs[0].cfgPath,
		env: append(os.Environ(),
			"BW_CLOCK="+fixedClock,
			"GIT_AUTHOR_DATE="+fixedClock,
			"GIT_COMMITTER_DATE="+fixedClock,
			"NO_COLOR=1",
			"BW_CONFIG="+envs[0].cfgPath,
			"GIT_CONFIG_GLOBAL=/dev/null",
			"GIT_CONFIG_SYSTEM=/dev/null",
		),
	}
...
func TestCFlagCollisionErrors(t *testing.T) {
	...
	// Inject a duplicate prefix in the registry by creating a second repo
	// with the same prefix and registering it.
	...
	twin.git("init")
	twin.git("config", "user.email", "test@beadwork.dev")
	twin.git("config", "user.name", "Test User")
	os.WriteFile(filepath.Join(otherPath, "README"), []byte("twin"), 0644)
	twin.git("add", ".")
...
func TestCrossRepoPrefixCollision(t *testing.T) {
	...
	// Create a second repo with the same prefix as envs[0] ("r0") and
	// register it, so the prefix resolver sees a collision.
	otherPath := filepath.Join(filepath.Dir(envs[0].dir), "twin")
	twin := &bwEnv{
		t:       t,
		dir:     otherPath,
